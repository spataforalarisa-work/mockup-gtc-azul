...
export default function HomePage() {
  ...
  const STATS = [
    { value: '53', label: en ? 'active companies' : 'empresas activas', color: 'text-blue-prime' },
    { value: '111', label: en ? 'professionals' : 'profesionales', color: 'text-blue-prime' },
    { value: '50%', label: en ? 'average savings' : 'ahorro promedio', color: 'text-gold' },
    { value: '95%', label: en ? 'retention' : 'retención', color: 'text-gold' },
  ]

  const ADVANTAGES = [
    {
      icon: Zap,
      title: en ? '15-day match' : 'Match en 15 días',
      description: en
        ? 'From the first call to your assistant operating. Technical tests, shortlist of 2-3 candidates, you choose.'
        : 'De la primera llamada a tu asistente operando. Pruebas técnicas, shortlist de 2-3 candidatos, tú eliges.',
    },
    {
      icon: Brain,
      title: en ? 'Rigorous selection' : 'Selección rigurosa',
      description: en
        ? 'Less than 5% acceptance rate. +11,000 candidates validated with technical and behavioral tests.'
        : 'Menos del 5% de tasa de aceptación. +11.000 candidatos validados con pruebas técnicas y conductuales.',
    },
    {
      icon: Handshake,
      title: 'Zero Overhead',
      description: en
        ? 'We hire, manage, and supervise. Your company gets a monthly invoice. Replacement guaranteed.'
        : 'Nosotros contratamos, gestionamos y supervisamos. Tu empresa recibe una factura mensual. Reemplazo garantizado.',
    },
  ]
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          <div className="lg:col-span-7">
            <h1 className="text-off-white font-headline text-4xl sm:text-5xl md:text-7xl leading-[1.1] mb-8">
              {en ? (
                <>Remote <span className="serif-italic text-gold">senior</span> talent for companies that want to grow without complications.</>
              ) : (
                <>Talento remoto{' '}<span className="serif-italic text-gold">senior</span>{' '}para empresas que quieren crecer sin complicaciones.</>
              )}
            </h1>
            <p className="text-white/60 text-lg md:text-2xl mb-12 max-w-2xl font-light">
              {en
                ? 'We connect companies with high-performance professionals in remote mode. Full selection, management, and supervision.'
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                {en ? 'For Companies' : 'Para Empresas'}
              ...
              >
                {en ? 'For Professionals' : 'Para Profesionales'}
              </Link>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative hidden lg:block">
            <div className="relative w-full aspect-[4/5] rounded-xl overflow-hidden shadow-2xl blue-overlay">
              <img
                alt={en ? 'Professional working remotely' : 'Profesional trabajando de forma remota'}
            ...
            <div className="absolute -bottom-8 -left-12 glass-card p-6 rounded-xl border border-white/10 shadow-2xl w-80 backdrop-blur-2xl">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h4 className="text-white font-headline text-lg">{en ? 'Verified Profile' : 'Perfil Verificado'}</h4>
                  <p className="text-white/40 text-[10px] font-label uppercase tracking-widest">{en ? 'Senior Admin Assistant' : 'Asistente Admin Senior'}</p>
                </div>
              </div>
              <div className="space-y-4 mb-6">
                <div className="flex flex-wrap gap-2">
                  <span className="px-2 py-1 bg-blue-prime/20 text-blue-light text-[9px] font-bold rounded uppercase tracking-wider">{en ? 'Advanced Excel' : 'Excel avanzado'}</span>
                  ...
                  <span className="px-2 py-1 bg-gold/20 text-gold text-[9px] font-bold rounded uppercase tracking-wider">{en ? 'Billing' : 'Facturación'}</span>
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400 font-label">{en ? 'SATISFACTION' : 'SATISFACCION'}</span>
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400 font-label">{en ? 'AVAILABILITY' : 'DISPONIBILIDAD'}</span>
                  <span className="text-blue-light font-bold flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-blue-prime animate-pulse" /> {en ? 'Immediate' : 'Inmediata'}
                  </span>
                </div>
              </div>
              <Link
              ...
              >
                {en ? 'View available profiles' : 'Ver perfiles disponibles'}
              </Link>
            </div>
          </div>
        </div>
      ...
      <RevealSection className="relative z-20 -mt-12 max-w-5xl mx-auto px-6 lg:px-8">
        <div className="bg-white rounded-xl py-10 lg:py-12 px-8 lg:px-12 grid grid-cols-2 md:grid-cols-4 gap-8 shadow-2xl border border-border-soft">
          {STATS.map((stat, i) => (
            <div key={stat.value} className={`text-center ${i > 0 ? 'md:border-l md:border-border-soft' : ''}`}>
      ...
      {/* LOGO TICKER */}
      <section className="py-16 lg:py-20 bg-off-white overflow-hidden" aria-label={en ? 'Clients' : 'Clientes'}>
        <div className="text-center mb-10">
          <span className="text-navy/70 text-xs font-label uppercase tracking-[0.3em]">
            {en ? 'Companies that trust us' : 'Empresas que confian en nosotros'}
          </span>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center relative z-10">
          <div className="relative order-2 lg:order-1">
            <div className="rounded-2xl overflow-hidden shadow-2xl blue-overlay">
              <img
                alt={en ? 'Team working remotely' : 'Equipo trabajando en remoto'}
            ...
            <div className="absolute top-8 lg:top-12 -right-4 lg:-right-12 bg-white p-6 lg:p-8 shadow-xl rounded-xl max-w-xs border-l-4 border-blue-prime">
              <p className="font-headline text-base lg:text-lg text-navy italic mb-4">
                {en
                  ? '"We select and manage high-performance talent so your company can focus on what matters."'
          ...
          <div className="order-1 lg:order-2">
            <span className="text-blue-prime text-xs font-label uppercase tracking-widest font-extrabold mb-6 block">
            ...
            <h2 className="text-navy font-headline text-4xl md:text-5xl lg:text-6xl leading-tight mb-10">
              {en ? (
                <>Humanizing the <span className="serif-italic text-blue-deep">global</span> connection.</>
              ) : (
            ...
            <div className="space-y-6 text-navy/80 font-light leading-relaxed text-lg">
              <p>
                {en
                  ? "We're not a placement agency. We're your operational partner: we select, hire, supervise, and replace if needed. You don't manage anything."
              ...
              <p>
                {en
                  ? 'With offices in Spain and Miami, we connect high-performance professionals with companies that need more operational capacity without inflating their local payroll.'
            ...
            <div className="mt-12">
              <Link to="/nosotros" className="flex items-center gap-4 group">
                ...
                <span className="font-label text-sm uppercase tracking-widest font-extrabold text-navy group-hover:text-blue-prime transition-colors">
                  {en ? 'Meet the team' : 'Conoce al equipo'}
                </span>
              </Link>
            </div>
          </div>
      ...
      <RevealSection className="py-24 lg:py-32 bg-off-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
          <div className="max-w-2xl mb-16 lg:mb-20">
            <span className="text-blue-prime text-xs font-label uppercase tracking-widest font-extrabold mb-6 block">
              {en ? 'Strategic Advantages' : 'Ventajas Estratégicas'}
            </span>
            <h2 className="text-navy font-headline text-4xl lg:text-5xl leading-tight">
              {en ? 'Why choose excellence.' : 'Por qué elegir la excelencia.'}
            </h2>
          </div>
